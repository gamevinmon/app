...
(() => {
  ...
  async function ensureMonadNetwork() {
    ...
    try {
      ...
      alert(
        "Please select Monad network (chainId 143) in MetaMask before using the dApp."
      );
      return false;
    }
  }
    ...
    try {
      ...
      if (swapDirection === "vinToMon") {
        ...
        if (vinBalanceBN.lt(amountBN)) {
          ...
          alert("Not enough VIN in wallet.");
          return;
        }
        ...
        if (monBalanceBN.lt(amountBN)) {
          ...
          alert("Not enough MON in wallet.");
          return;
        }
        if (statusEl)
  ...
  // ===== Dice Play Logic (Updated to prevent CALL_EXCEPTION) =====
  async function onDicePlay() {
    ...
    try {
      ...
      // Read current values: minBet, bankroll, allowance, balance
      ...
      if (amountBN.lt(minBetBN)) {
        ...
        alert(`Minimum bet is ${minStr} VIN.`);
        return;
      }
      ...
      // 3) Check bankroll (estimate: needs at least amount * 2 for payout)
      ...
      if (bankrollBN.lt(neededPayout)) {
        ...
        alert(
          `Reward pool is too small. Current bankroll: ${poolStr} VIN.`
        );
        return;
      }
      ...
      if (allowanceBN.lt(amountBN)) {
        ...
        alert(
          `Allowance is too low (${allowStr} VIN). Please approve first (${needStr} VIN).`
        );
        return;
      }
      ...
      // 5) Estimate gas before sending TX
      let gasLimit;
      try {
        ...
        alert(
          "This bet will be reverted, so not sending to the network.\n" + (reason || "")
        );
        return;
      }
