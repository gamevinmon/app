...
(() => {
  ...
  // Approve tối đa cho Dice (10 triệu VIN)
  ...
  // ===== Global State =====
  let rpcProvider = null;       // read-only provider (RPC)
  ...
  function formatUnitsSafe(value, decimals = 18, precision = 4) {
    try {
      const str = ethers.utils.formatUnits(value, decimals);
  ...
  function formatVin(bn, precision = 4) {
    return formatUnitsSafe(bn, VIN_DECIMALS, precision);
  }
  function formatMon(bn, precision = 4) {
    return formatUnitsSafe(bn, MON_DECIMALS, precision);
  }
  ...
  async function ensureMonadNetwork() {
    ...
    try {
      ...
      alert(
        "Vui lòng chọn mạng Monad trong MetaMask (chainId 143) trước khi dùng dApp."
      );
      return false;
    }
  }
  ...
  async function updateDicePool() {
    try {
      ...
      const poolStr = formatVin(bankroll);
  ...
  function updateSwapDirectionUI() {
    const tabVinToMon = $("tabVinToVin"); // typo-safe guard
    const tabVinToMonReal = $("tabVinToMon");
    const tabMonToVin = $("tabMonToVin");

    const tabA = tabVinToMonReal || tabVinToMon;

    if (tabA && tabMonToVin) {
      tabA.classList.remove("swap-tab-active");
      tabMonToVin.classList.remove("swap-tab-active");

      if (swapDirection === "vinToMon") {
        tabA.classList.add("swap-tab-active");
      } else {
  ...
  function setSwapMax() {
    ...
    if (swapDirection === "vinToMon") {
      fromInput.value = formatVin(vinBalanceBN, 6);
    } else {
      // Trừ một chút gas cho MON (giữ lại ~0.002 MON)
      ...
      fromInput.value = formatMon(usable, 6);
    }
    updateSwapToAmount();
  }
    ...
    try {
      ...
      if (swapDirection === "vinToMon") {
        ...
        if (allowanceBN.lt(amountBN)) {
          if (statusEl)
            statusEl.textContent = "Approving VIN for Swap (please confirm in MetaMask)...";
  ...
  function setDiceCoinsPattern(resultEven) {
    ...
    // Predefined patterns (white/red)
    const evenPatterns = [
      ["white", "white", "white", "white"], // 0 đỏ
      ["white", "red", "red", "white"],     // 2 đỏ
      ["red", "white", "white", "red"]      // 2 đỏ
    ];
    const oddPatterns = [
      ["red", "white", "white", "white"],   // 1 đỏ
      ["white", "red", "white", "white"],   // 1 đỏ
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (pattern[i] === "red") {
        c.classList.add("dice-coin-red");
      } else {
        c.classList.add("dice-coin-white");
      }
    }
  }
  ...
  function onGuessButtonClick(isEven) {
    ...
    if (evenBtn && oddBtn) {
      if (isEven) {
        evenBtn.classList.add("dice-guess-active");
        oddBtn.classList.remove("dice-guess-active");
      } else {
        oddBtn.classList.add("dice-guess-active");
  ...
  async function updateDiceLimitsAndAllowance() {
    try {
      ...
      const minBetStr = formatVin(minBet);
      ...
      if (currentAccount) {
        ...
        const allowanceStr = formatVin(allowance);
  ...
  async function onDiceApprove() {
    ...
    try {
      ...
      if (statusEl)
        statusEl.textContent = "Approving VIN for Dice (please confirm in MetaMask)...";
  ...
  function onDiceQuickButtons(action) {
    ...
    input.value = formatVin(currentBN, 6);
  }

  async function onDiceRefreshLast() {
    // hiện chỉ dùng local (không query logs on-chain)
  ...
  async function onDicePlay() {
    ...
    if (diceInFlight) {
      // tránh spam khi tx trước chưa xong
      return;
    }
    ...
    try {
      ...
      // Lấy dữ liệu để tránh CALL_EXCEPTION
      ...
      // bankroll đủ trả 2x (cộng chút buffer cho fee nếu có)
      ...
      if (receipt.status !== 1) {
        if (statusEl) {
          statusEl.textContent =
            "Dice transaction reverted. Check explorer or try smaller bet.";
        }
        console.warn("Dice tx status != 1", receipt);
        return;
      }

      // Parse sự kiện Played
      ...
      // Quan trọng: luôn reset để không phải F5 sau khi lỗi
      diceInFlight = false;
  ...
  function initSwapEvents() {
    ...
    if (fromInput)
      fromInput.addEventListener("input", () => updateSwapToAmount());
  ...
  function initDiceEvents() {
    ...
    if (maxBtn)
      maxBtn.addEventListener("click", () => {
        // max = min(vinBalance, bankroll/2)
        const maxByBalance = vinBalanceBN;
        const maxByBankroll = diceBankrollBN.div(2);
        const maxBN = maxByBalance.lt(maxByBankroll)
          ? maxByBalance
          : maxByBankroll;
        const input = $("diceBetAmount");
        if (input) input.value = formatVin(maxBN, 6);
      });
    if (repeatBtn)
  ...
  async function initApp() {
    try {
      ...
      // đọc pool dice (bankroll) khi chưa connect ví
      await updateDicePool();
